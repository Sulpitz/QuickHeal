...
local function Message(text, kind, duration)
	...
	if kind == L["Healing"] and QHV.MessageScreenCenterHealing then
		UIErrorsFrame:AddMessage(text, 0.1, 1, 0.1, 1, duration or 2)
	elseif kind == L["Info"] and QHV.MessageScreenCenterInfo then
		UIErrorsFrame:AddMessage(text, 0.1, 0.1, 1, 1, duration or 2)
	elseif kind == L["Blacklist"] and QHV.MessageScreenCenterBlacklist then
		UIErrorsFrame:AddMessage(text, 1, 0.9, 0, 1, duration or 2)
	elseif kind == L["Error"] and QHV.MessageScreenCenterError then
		UIErrorsFrame:AddMessage(text, 1, 0.1, 0.1, 1, duration or 2)
	end
	-- Deliver message to chat window
	if kind == L["Healing"] and QHV.MessageChatWindowHealing then
		writeLine(text, 0.1, 1, 0.1)
	elseif kind == L["Info"] and QHV.MessageChatWindowInfo then
		writeLine(text, 0.1, 0.1, 1)
	elseif kind == L["Blacklist"] and QHV.MessageChatWindowBlacklist then
		writeLine(text, 1, 0.9, 0.2)
	elseif kind == L["Error"] and QHV.MessageChatWindowError then
...
local function UnitIsHealable(unit, explain)
	if UnitExists(unit) then
		if EvaluateUnitCondition(unit, UnitIsFriend("player", unit), L["is not a friend"], explain) then
			return false
		end
		if EvaluateUnitCondition(unit, not UnitIsEnemy(unit, "player"), L["is an enemy"], explain) then
			return false
		end
		if EvaluateUnitCondition(unit, not UnitCanAttack("player", unit), L["can be attacked by player"], explain) then
			return false
		end
		if EvaluateUnitCondition(unit, UnitIsConnected(unit), L["is not connected"], explain) then
			return false
		end
		if EvaluateUnitCondition(unit, not UnitIsDeadOrGhost(unit), L["is dead or ghost"], explain) then
			return false
		end
		if EvaluateUnitCondition(unit, UnitIsVisible(unit), L["is not visible to client"], explain) then
			return false
		end
	else
		return false
...
local function Notification(unit, spellName)
	...
	if rand == 1 then
		read = string.format(L["%s is looking pale, gonna heal you with %s."], unitName, spellName)
	end
	if rand == 2 then
		read = string.format(L["%s doesn't look so hot, healing with %s."], unitName, spellName)
	end
	if rand == 3 then
		read = string.format(L["I know it's just a flesh wound %s, but I'm healing you with %s."], unitName, spellName)
	end
	if rand == 4 then
		read = string.format(L["Oh great, %s is bleeding all over, %s should take care of that."], unitName, spellName)
	end
	if rand == 5 then
		read = string.format(L["Death is near %s... or is it? Perhaps a heal with %s will keep you with us."], unitName, spellName)
	end
	if rand == 6 then
		read = string.format(L["%s, lack of health got you down? %s to the rescue!"], unitName, spellName)
	end
	if rand == 7 then
	...
	if PlayerRace == "Orc" then
		...
		if rand == 9 then
			read = string.format(L["Loktar! %s is being healed with %s."], unitName, spellName)
		end
		if rand == 10 then
	...
	if PlayerRace == "Tauren" then
		...
		if rand == 9 then
			read = string.format(L["Ancestors, save %s with %s."], unitName, spellName)
		end
		if rand == 10 then
	...
	if PlayerRace == "Troll" then
		...
		if rand == 9 then
			read = string.format(L["Haha! %s keeps dyin' an da %s voodoo, keeps bringin' em back!."], unitName, spellName)
		end
		if rand == 10 then
	...
	if PlayerRace == "NightElf" then
		...
		if rand == 9 then
			read = string.format(L["Remain vigilent %s, the Goddess' %s shall revitalize you!"], unitName, spellName)
		end
		if rand == 10 then
			read = string.format(L["By Elune's grace I grant you this %s, %s."], spellName, unitName)
		end
	end
