...
function QuickHeal_Priest_GetRatioHealthyExplanation()
	if QuickHealVariables.RatioHealthyPriest >= QuickHealVariables.RatioFull then
		...
		if QuickHealVariables.RatioHealthyPriest > 0 then
			return BS["Flash Heal"]
				.. L[" will be used in combat if the target has less than "]
...
function QuickHeal_Priest_FindSpellToUse(Target)
	...
	if quickHealHealMode == 1 then
		...
		if not InCombat or TargetIsHealthy or maxRankFH < 1 then
			...
			if Health < QuickHealVariables.RatioFull then
				SpellID = SpellIDsLH[1]
				HealSize = 53 * shMod + healMod15 * PF1 -- Default to LH
				if healneed > (84 * shMod + healMod20 * PF4) * k and ManaLeft >= 45 * ihMod and maxRankLH >= 2 and quickHealDownRankNH >= 2 then
					SpellID = SpellIDsLH[2]
					HealSize = 84 * shMod + healMod20 * PF4
				end
				if healneed > (154 * shMod + healMod25 * PF10) * K and ManaLeft >= 75 * ihMod and maxRankLH >= 3 and quickHealDownRankNH >= 3 then
					SpellID = SpellIDsLH[3]
					HealSize = 154 * shMod + healMod25 * PF10
				end
				if healneed > (318 * shMod + healMod30 * PF18) * K and ManaLeft >= 155 * ihMod and maxRankH >= 1 and quickHealDownRankNH >= 4 then
					SpellID = SpellIDsH[1]
					HealSize = 318 * shMod + healMod30 * PF18
				end
				if healneed > (460 * shMod + healMod30) * K and ManaLeft >= 205 * ihMod and maxRankH >= 2 and quickHealDownRankNH >= 5 then
					SpellID = SpellIDsH[2]
					HealSize = 460 * shMod + healMod30
				end
				if healneed > (604 * shMod + healMod30) * K and ManaLeft >= 255 * ihMod and maxRankH >= 3 and quickHealDownRankNH >= 6 then
					SpellID = SpellIDsH[3]
					HealSize = 604 * shMod + healMod30
				end
				if healneed > (758 * shMod + healMod30) * K and ManaLeft >= 305 * ihMod and maxRankH >= 4 and quickHealDownRankNH >= 7 then
					SpellID = SpellIDsH[4]
					HealSize = 758 * shMod + healMod30
				end
				if healneed > (956 * shMod + healMod30) * K and ManaLeft >= 370 * ihMod and maxRankGH >= 1 and quickHealDownRankNH >= 8 then
					SpellID = SpellIDsGH[1]
					HealSize = 956 * shMod + healMod30
				end
				if healneed > (1219 * shMod + healMod30) * K and ManaLeft >= 455 * ihMod and maxRankGH >= 2 and quickHealDownRankNH >= 9 then
					SpellID = SpellIDsGH[2]
					HealSize = 1219 * shMod + healMod30
				end
				if healneed > (1523 * shMod + healMod30) * K and ManaLeft >= 545 * ihMod and maxRankGH >= 3 and quickHealDownRankNH >= 10 then
					SpellID = SpellIDsGH[3]
					HealSize = 1523 * shMod + healMod30
				end
				if healneed > (1902 * shMod + healMod30) * K and ManaLeft >= 655 * ihMod and maxRankGH >= 4 and quickHealDownRankNH >= 11 then
					SpellID = SpellIDsGH[4]
					HealSize = 1902 * shMod + healMod30
				end
				if healneed > (2080 * shMod + healMod30) * K and ManaLeft >= 710 * ihMod and maxRankGH >= 5 and quickHealDownRankNH >= 12 then
					SpellID = SpellIDsGH[5]
					HealSize = 2080 * shMod + healMod30
				end
			end
		else
			...
			if Health < QuickHealVariables.RatioFull then
				SpellID = SpellIDsFH[1]
				HealSize = 215 * shMod + healMod15 -- Default to FH
				if healneed > (286 * shMod + healMod15) * k and ManaLeft >= 155 and maxRankFH >= 2 and quickHealDownRankFH >= 2 then
					SpellID = SpellIDsFH[2]
					HealSize = 286 * shMod + healMod15
				end
				if healneed > (360 * shMod + healMod15) * k and ManaLeft >= 185 and maxRankFH >= 3 and quickHealDownRankFH >= 3 then
					SpellID = SpellIDsFH[3]
					HealSize = 360 * shMod + healMod15
				end
				if healneed > (439 * shMod + healMod15) * k and ManaLeft >= 215 and maxRankFH >= 4 and quickHealDownRankFH >= 4 then
					SpellID = SpellIDsFH[4]
					HealSize = 439 * shMod + healMod15
				end
				if healneed > (567 * shMod + healMod15) * k and ManaLeft >= 265 and maxRankFH >= 5 and quickHealDownRankFH >= 5 then
					SpellID = SpellIDsFH[5]
					HealSize = 567 * shMod + healMod15
				end
				if healneed > (704 * shMod + healMod15) * k and ManaLeft >= 315 and maxRankFH >= 6 and quickHealDownRankFH >= 6 then
					SpellID = SpellIDsFH[6]
					HealSize = 704 * shMod + healMod15
				end
				if healneed > (885 * shMod + healMod15) * k and ManaLeft >= 380 and maxRankFH >= 7 and quickHealDownRankFH >= 7 then
					SpellID = SpellIDsFH[7]
					HealSize = 885 * shMod + healMod15
				end
			end
		end
	...
	elseif quickHealHealMode == 3 then
		...
		--Check NormalHeal
		SpellIDNH = SpellIDsLH[1]
		HealSizeNH = 53 * shMod + healMod15 * PF1 -- Default to LH
		if healneed > (84 * shMod + healMod20 * PF4) * k and ManaLeft >= 45 * ihMod and maxRankLH >= 2 and quickHealDownRankNH >= 2 then
			SpellIDNH = SpellIDsLH[2]
			HealSizeNH = 84 * shMod + healMod20 * PF4
		end
		if healneed > (154 * shMod + healMod25 * PF10) * K and ManaLeft >= 75 * ihMod and maxRankLH >= 3 and quickHealDownRankNH >= 3 then
			SpellIDNH = SpellIDsLH[3]
			HealSizeNH = 154 * shMod + healMod25 * PF10
		end
		if healneed > (318 * shMod + healMod30 * PF18) * K and ManaLeft >= 155 * ihMod and maxRankH >= 1 and quickHealDownRankNH >= 4 then
			SpellIDNH = SpellIDsH[1]
			HealSizeNH = 318 * shMod + healMod30 * PF18
		end
		if healneed > (460 * shMod + healMod30) * K and ManaLeft >= 205 * ihMod and maxRankH >= 2 and quickHealDownRankNH >= 5 then
			SpellIDNH = SpellIDsH[2]
			HealSizeNH = 460 * shMod + healMod30
		end
		if healneed > (604 * shMod + healMod30) * K and ManaLeft >= 255 * ihMod and maxRankH >= 3 and quickHealDownRankNH >= 6 then
			SpellIDNH = SpellIDsH[3]
			HealSizeNH = 604 * shMod + healMod30
		end
		if healneed > (758 * shMod + healMod30) * K and ManaLeft >= 305 * ihMod and maxRankH >= 4 and quickHealDownRankNH >= 7 then
			SpellIDNH = SpellIDsH[4]
			HealSizeNH = 758 * shMod + healMod30
		end
		if healneed > (956 * shMod + healMod30) * K and ManaLeft >= 370 * ihMod and maxRankGH >= 1 and quickHealDownRankNH >= 8 then
			SpellIDNH = SpellIDsGH[1]
			HealSizeNH = 956 * shMod + healMod30
		end
		if healneed > (1219 * shMod + healMod30) * K and ManaLeft >= 455 * ihMod and maxRankGH >= 2 and quickHealDownRankNH >= 9 then
			SpellIDNH = SpellIDsGH[2]
			HealSizeNH = 1219 * shMod + healMod30
		end
		if healneed > (1523 * shMod + healMod30) * K and ManaLeft >= 545 * ihMod and maxRankGH >= 3 and quickHealDownRankNH >= 10 then
			SpellIDNH = SpellIDsGH[3]
			HealSizeNH = 1523 * shMod + healMod30
		end
		if healneed > (1902 * shMod + healMod30) * K and ManaLeft >= 655 * ihMod and maxRankGH >= 4 and quickHealDownRankNH >= 11 then
			SpellIDNH = SpellIDsGH[4]
			HealSizeNH = 1902 * shMod + healMod30
		end
		if healneed > (2080 * shMod + healMod30) * K and ManaLeft >= 710 * ihMod and maxRankGH >= 5 and quickHealDownRankNH >= 12 then
			SpellIDNH = SpellIDsGH[5]
			HealSizeNH = 2080 * shMod + healMod30
		end

		--Check Flash Heal
		SpellIDFH = SpellIDsFH[1]
		HealSizeFH = 215 * shMod + healMod15 -- Default to FH
		if healneed > (286 * shMod + healMod15) * k and ManaLeft >= 155 and maxRankFH >= 2 and quickHealDownRankFH >= 2 then
			SpellIDFH = SpellIDsFH[2]
			HealSizeFH = 286 * shMod + healMod15
		end
		if healneed > (360 * shMod + healMod15) * k and ManaLeft >= 185 and maxRankFH >= 3 and quickHealDownRankFH >= 3 then
			SpellIDFH = SpellIDsFH[3]
			HealSizeFH = 360 * shMod + healMod15
		end
		if healneed > (439 * shMod + healMod15) * k and ManaLeft >= 215 and maxRankFH >= 4 and quickHealDownRankFH >= 4 then
			SpellIDFH = SpellIDsFH[4]
			HealSizeFH = 439 * shMod + healMod15
		end
		if healneed > (567 * shMod + healMod15) * k and ManaLeft >= 265 and maxRankFH >= 5 and quickHealDownRankFH >= 5 then
			SpellIDFH = SpellIDsFH[5]
			HealSizeFH = 567 * shMod + healMod15
		end
		if healneed > (704 * shMod + healMod15) * k and ManaLeft >= 315 and maxRankFH >= 6 and quickHealDownRankFH >= 6 then
			SpellIDFH = SpellIDsFH[6]
			HealSizeFH = 704 * shMod + healMod15
		end
		if healneed > (885 * shMod + healMod15) * k and ManaLeft >= 380 and maxRankFH >= 7 and quickHealDownRankFH >= 7 then
			SpellIDFH = SpellIDsFH[7]
			HealSizeFH = 885 * shMod + healMod15
		end

		--Check Prayer of Heal
		writePriestLine(L["healneedGrp: "] .. healneedGrp)
		SpellIDGrp = SpellIDPoH[1]
		HealSizePoH = 0
		if healneedGrp > (311 * shMod + healMod30 / 2) * k and ManaLeft >= 410 and maxRankPoH >= 1 and quickHealDownRankNH >= 8 then
			SpellIDGrp = SpellIDPoH[1]
			HealSizePoH = 311 * shMod + healMod30 / 2
		end
		if healneedGrp > (458 * shMod + healMod30 / 2) * k and ManaLeft >= 560 and maxRankPoH >= 1 and quickHealDownRankNH >= 9 then
			SpellIDGrp = SpellIDPoH[2]
			HealSizePoH = 458 * shMod + healMod30 / 2
		end
		if healneedGrp > (676 * shMod + healMod30 / 2) * k and ManaLeft >= 770 and maxRankPoH >= 1 and quickHealDownRankNH >= 10 then
			SpellIDGrp = SpellIDPoH[3]
			HealSizePoH = 676 * shMod + healMod30 / 2
		end
		if healneedGrp > (965 * shMod + healMod30 / 2) * k and ManaLeft >= 1030 and maxRankPoH >= 1 and quickHealDownRankNH >= 11 then
			SpellIDGrp = SpellIDPoH[4]
			HealSizePoH = 965 * shMod + healMod30 / 2
		end
		if healneedGrp > (1070 * shMod + healMod30 / 2) * k and ManaLeft >= 1070 and maxRankPoH >= 1 and quickHealDownRankNH >= 12 then
			SpellIDGrp = SpellIDPoH[5]
			HealSizePoH = 1070 * shMod + healMod30 / 2
		end
