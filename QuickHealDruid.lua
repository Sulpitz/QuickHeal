...
function QuickHeal_Druid_FindSpellToUse(Target)
	...
	if not InCombat or TargetIsHealthy or maxRankRG < 1 then
		...
		if Health < RatioFull then
			...
			HealSize = 44 * gnMod + healMod15 * PF1 -- Default to rank 1
			if healneed > (100 * gnMod + healMod20 * PF8) * k and ManaLeft >= 55 * tsMod * mgMod and maxRankHT >= 2 then
				SpellID = SpellIDsHT[2]
				HealSize = 100 * gnMod + healMod20 * PF8
			end
			if healneed > (219 * gnMod + healMod25 * PF14) * K and ManaLeft >= 110 * tsMod * mgMod and maxRankHT >= 3 then
				SpellID = SpellIDsHT[3]
				HealSize = 219 * gnMod + healMod25 * PF14
			end
			if healneed > (404 * gnMod + healMod30) * K and ManaLeft >= 185 * tsMod * mgMod and maxRankHT >= 4 then
				SpellID = SpellIDsHT[4]
				HealSize = 404 * gnMod + healMod30
			end
			if healneed > (633 * gnMod + healMod35) * K and ManaLeft >= 270 * tsMod * mgMod and maxRankHT >= 5 then
				SpellID = SpellIDsHT[5]
				HealSize = 633 * gnMod + healMod35
			end
			if healneed > (818 * gnMod + healMod35) * K and ManaLeft >= 335 * tsMod * mgMod and maxRankHT >= 6 then
				SpellID = SpellIDsHT[6]
				HealSize = 818 * gnMod + healMod35
			end
			if healneed > (1028 * gnMod + healMod35) * K and ManaLeft >= 405 * tsMod * mgMod and maxRankHT >= 7 then
				SpellID = SpellIDsHT[7]
				HealSize = 1028 * gnMod + healMod35
			end
			if healneed > (1313 * gnMod + healMod35) * K and ManaLeft >= 495 * tsMod * mgMod and maxRankHT >= 8 then
				SpellID = SpellIDsHT[8]
				HealSize = 1313 * gnMod + healMod35
			end
			if healneed > (1656 * gnMod + healMod35) * K and ManaLeft >= 600 * tsMod * mgMod and maxRankHT >= 9 then
				SpellID = SpellIDsHT[9]
				HealSize = 1656 * gnMod + healMod35
			end
			if healneed > (2060 * gnMod + healMod35) * K and ManaLeft >= 720 * tsMod * mgMod and maxRankHT >= 10 then
				SpellID = SpellIDsHT[10]
				HealSize = 2060 * gnMod + healMod35
			end
			if healneed > (2472 * gnMod + healMod35) * K and ManaLeft >= 800 * tsMod * mgMod and maxRankHT >= 11 then
				SpellID = SpellIDsHT[11]
				HealSize = 2472 * gnMod + healMod35
			end
		end
	else
		...
		if Health < RatioFull then
			...
			HealSize = 91 * gnMod + healModRG * PFRG1 -- Default to rank 1
			if healneed > (176 * gnMod + healModRG * PFRG2) * k and ManaLeft >= 205 * mgMod and maxRankRG >= 2 then
				SpellID = SpellIDsRG[2]
				HealSize = 176 * gnMod + healModRG * PFRG2
			end
			if healneed > (257 * gnMod + healModRG) * k and ManaLeft >= 280 * mgMod and maxRankRG >= 3 then
				SpellID = SpellIDsRG[3]
				HealSize = 257 * gnMod + healModRG
			end
			if healneed > (339 * gnMod + healModRG) * k and ManaLeft >= 350 * mgMod and maxRankRG >= 4 then
				SpellID = SpellIDsRG[4]
				HealSize = 339 * gnMod + healModRG
			end
			if healneed > (431 * gnMod + healModRG) * k and ManaLeft >= 420 * mgMod and maxRankRG >= 5 then
				SpellID = SpellIDsRG[5]
				HealSize = 431 * gnMod + healModRG
			end
			if healneed > (543 * gnMod + healModRG) * k and ManaLeft >= 510 * mgMod and maxRankRG >= 6 then
				SpellID = SpellIDsRG[6]
				HealSize = 543 * gnMod + healModRG
			end
			if healneed > (686 * gnMod + healModRG) * k and ManaLeft >= 615 * mgMod and maxRankRG >= 7 then
				SpellID = SpellIDsRG[7]
				HealSize = 686 * gnMod + healModRG
			end
			if healneed > (857 * gnMod + healModRG) * k and ManaLeft >= 740 * mgMod and maxRankRG >= 8 then
				SpellID = SpellIDsRG[8]
				HealSize = 857 * gnMod + healModRG
			end
			if healneed > (1061 * gnMod + healModRG) * k and ManaLeft >= 880 * mgMod and maxRankRG >= 9 then
				SpellID = SpellIDsRG[9]
				HealSize = 1061 * gnMod + healModRG
			end
		end
	end
