...
function QuickClick_PlayerFrame_OnClick(button)
	if not QuickClick(button, this.unit) then
		QuickClick_OldOnClick.PlayerFrame(button)
	end
end
-- PetFrame
function QuickClick_PetFrame_OnClick(button)
	if not QuickClick(button, this.unit) then
		QuickClick_OldOnClick.PetFrame(button)
	end
end
-- TargetFrame
function QuickClick_TargetFrame_OnClick(button)
	if not QuickClick(button, this.unit) then
		QuickClick_OldOnClick.TargetFrame(button)
	end
end
-- TargetofTarget
function QuickClick_TargetofTarget_OnClick(button)
	if not QuickClick(button, this.unit) then
		QuickClick_OldOnClick.TargetofTarget(button)
	end
end
-- PartyMemberFrame
function QuickClick_PartyMemberFrame_OnClick(partyFrame)
	if not QuickClick(arg1, this.unit) then
		QuickClick_OldOnClick.PartyMemberFrame(partyFrame)
	end
end
-- PartyMemberPetFrame
function QuickClick_PartyMemberPetFrame_OnClick()
	if not QuickClick(arg1, this.unit) then
		QuickClick_OldOnClick.PartyMemberPetFrame()
	end
end

-- EasyRaid
function QuickClick_ER_RaidPulloutButton_OnClick()
	if not QuickClick(arg1, this.unit or this:GetParent().unit) then
		QuickClick_OldOnClick.ER_RaidPulloutButton()
	end
end
function QuickClick_ER_MainTankButton_OnClick()
	if not QuickClick(arg1, this.unit or this:GetParent().unit) then
		QuickClick_OldOnClick.ER_MainTankButton()
	end
end
...
function QuickClick_DUF_UnitFrame_OnClick(button)
	if not QuickClick(button, this.unit) then
		QuickClick_OldOnClick.DUF_UnitFrame(button)
	end
end
function QuickClick_DUF_Element_OnClick(button)
	if not QuickClick(button, this.unit or this:GetParent().unit) then
		QuickClick_OldOnClick.DUF_Element(button)
	end
end
